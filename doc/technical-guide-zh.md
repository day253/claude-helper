...
| `buildClaudeEnv` | 生成待写入的 `ANTHROPIC_*` 键值；若配置 `default_model`，除覆盖 `ANTHROPIC_MODEL` 外，同步覆盖 `ANTHROPIC_SMALL_FAST_MODEL` 与 `ANTHROPIC_DEFAULT_{SONNET,OPUS,HAIKU}_MODEL`（Moonshot 另同步 `CLAUDE_CODE_SUBAGENT_MODEL`）；**glm / zai** 且本机 `claude --version` 为 2.1.69–2.1.x 时另注入 `ENABLE_TOOL_SEARCH=0`、`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1`（见 `claude-code-version.ts`） |
